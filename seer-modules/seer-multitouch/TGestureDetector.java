 ...
 * @author mzechner */
public class TGestureDetector extends InputAdapter {
	 ...
	 * maxFlingDelay=0.15f. */
	public TGestureDetector (GestureListener listener) {
	...
	public TGestureDetector (float halfTapSquareSize, float tapCountInterval, float longPressDuration, float maxFlingDelay,
	...
	public boolean touchDown (int x, int y, int pointer, int button) {
		return touchDown((float)x, (float)y, pointer, button, 0);
	}

	public boolean touchDown (float x, float y, int pointer, int button, long time) {
		...
		if (pointer == 0) {
			pointer1.set(x, y);
			gestureStartTime = time;  //Gdx.input.getCurrentEventTime();
	...
	public boolean touchDragged (int x, int y, int pointer) {
		return touchDragged((float)x, (float)y, pointer, 0);
	}

	public boolean touchDragged (float x, float y, int pointer, long time) {
		if (pointer > 1) return false;
	...
	public boolean touchUp (int x, int y, int pointer, int button) {
		return touchUp((float)x, (float)y, pointer, button, 0);
	}

	public boolean touchUp (float x, float y, int pointer, int button, long timestamp) {
		if (pointer > 1) return false;
		...
		if (inTapSquare) {
			...
			if (pointer == 0) {
				...
				tracker.start(pointer2.x, pointer2.y, timestamp);
			} else {
				// second pointer has lifted off, set up panning to use the first pointer...
				tracker.start(pointer1.x, pointer1.y, timestamp);
			}
		} else {
			gestureStartTime = 0;
			// handle fling
