  ...
  object Ease {
    ...
    @inline def expoIn(t:Float) = math.pow(2, 10 * (t-1))
    @inline def expoOut(t:Float) = -math.pow(2, -10 * t) + 1
    ...
    @inline def sine(t:Float) = -0.5 * (math.cos(t * math.Pi) - 1)
