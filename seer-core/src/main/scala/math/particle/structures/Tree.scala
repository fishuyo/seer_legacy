...
object TreeNode {
  ...
  def apply( p:Option[TreeNode], d:Float=1.f, t:Float=.2f, rel:Quat=Quat() ) = new TreeNode(p){
    quat = (if(p.isDefined) rel*p.get.quat else rel)
    lQuat = quat
    relQuat = rel
...
class TreeNode( parent: Option[TreeNode] ) extends Stick {
  ...
  quat = Quat().fromEuler(Vec3(math.Pi/2,0,0))
  lQuat = Quat().fromEuler(Vec3(math.Pi/2,0,0))
  ...
  def draw( ){ //v:Array[Float], idx:Int ):Int = {
    ...
    TreeNode.glprimitive.pose.pos = position
		TreeNode.glprimitive.pose.quat = quat
  ...
  override def step( dt: Float ) = {
    ...
    children.foreach( (n) => {
      n.spring.zeroQuat = n.relQuat * quat
      n.step(dt)
    })
