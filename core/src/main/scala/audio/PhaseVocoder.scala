...
class PhaseVocoder extends AudioSource {
  ...
  var convert = true
  def convert(b:Boolean): Unit ={ convert = b }

  def timeShift(f:Float): Unit ={ timeShift = f}
  def pitchShift(f:Float): Unit ={ pitchShift = f}
  def gain(f:Float): Unit ={ gain = f}

  def clear(): Unit ={ spectrumData = null }

  def setBounds(b1:Float,b2:Float): Unit ={
  ...
  def setSamples(buffer:Array[Float], size:Int): Unit ={
