...
trait OSCReceiver {
  ...
  // dummy function
  def empty(s:String)(v:Seq[Any]) = {} //println(s)}
  ...
  def handleMessage(msg:Message, addr:SocketAddress): Unit ={
    client = addr
    phandlers.foreach(_(msg))
    msg match {
      case Message( name, vals @ _* ) =>
        callbacks.getOrElse(name, empty(name)_ )(vals.asInstanceOf[Seq[Any]])
        ghandlers.foreach{ case f => f(name,vals.asInstanceOf[Seq[Any]]) }
...
trait OSCSender {
  ...
  def send(address:String, f1:Float, f2:Float): Unit ={ send(Message(address,f1,f2)) }
  def send(address:String, f1:Float, f2:Float, f3:Float): Unit ={ send(Message(address,f1,f2,f3)) }
  def send(address:String, f1:Float, f2:Float, f3:Float, f4:Float): Unit ={ send(Message(address,f1,f2,f3,f4)) }
  def send(address:String, s1:String, s2:String): Unit ={ send(Message(address,s1,s2)) }

  /** Connect and send message */
  def send(ip:String = "localhost", port:Int=8000, address:String, value:Any): Unit ={
    connect(ip,port)
    send(address,value)                        
  }

  def startBundle(count:Int=30): Unit ={ maxBundleLength = count; bundle = true }
  def endBundle(): Unit ={
    bundle = false
