...
object Script extends SeerScript {
  ...
  model2.material = new SpecularMaterial
  ...
  override def animate(dt:Float){
    ...
    if(dirty){
      ...
      // model2.mesh.clear
      model2.mesh = model.makeMesh(null,0.2f,4)
      dirty = false
    }
    FPS.print
  }
  ...
  Trackpad.bind( (t)=>{ 
    t.count match {
      case 1 =>
      case 2 => rotz += t.vel.x; modelGenerator.set("rotz",rotz); dirty = true
      case 3 => roty += t.vel.y; modelGenerator.set("roty",roty); dirty = true
      case _ => ()
    } 
  })
