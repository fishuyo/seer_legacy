...
void main() 
{
  ...
  float depth   = texture2D(u_texture0, v_texCoords).a;
  vec3 normal   = texture2D(u_texture0, v_texCoords).rgb;
  ...
  diff += texture2D(u_texture0, vec2(coord.x+d, coord.y)).a;
  diff += texture2D(u_texture0, vec2(coord.x-d, coord.y)).a;
  diff += texture2D(u_texture0, vec2(coord.x, coord.y+d)).a;
  diff += texture2D(u_texture0, vec2(coord.x, coord.y-d)).a;
  float edge = 1.0 - step(0.0001,abs(diff));

  //average normals;
  //normal += texture2D(u_texture0, vec2(coord.x+d, coord.y)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x-d, coord.y)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x, coord.y+d)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x, coord.y-d)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x+d, coord.y+d)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x-d, coord.y+d)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x-d, coord.y+d)).rgb;
  //normal += texture2D(u_texture0, vec2(coord.x+d, coord.y-d)).rgb;
  //normal /= 9.0;
  ...
  float spec = 0.0; //pow( max( dot(reflectVec, eye), 0.0), 10.0); 
  //spec = step(.3, spec);
